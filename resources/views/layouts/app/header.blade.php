...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="dark">
    ...
    <body class="min-h-screen bg-white dark:bg-zinc-800">
        <flux:header container class="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <x-app-logo href="{{ auth()->user()?->hasRole('Admin') ? route('admin.dashboard') : route('home') }}" wire:navigate />

            @role('Admin')
            <flux:navbar class="-mb-px max-lg:hidden">
                <flux:navbar.item icon="presentation-chart-bar" :href="route('admin.dashboard')" :current="request()->routeIs('admin.dashboard')" wire:navigate>
                    {{ __('Dashboard') }}
                </flux:navbar.item>
            </flux:navbar>
            @endrole
